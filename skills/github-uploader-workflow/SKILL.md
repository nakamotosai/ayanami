...
2. **Prepare the isolated workspace.** Change directory into the dedicated workspace (e.g., `/home/ubuntu/.openclaw/workspace/github-upload-workspace`). Ensure it points to the correct Git remote; run `git status -sb` to confirm a clean baseline, and `git remote -v` to verify the repo URL.
